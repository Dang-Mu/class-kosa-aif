    ...
    >
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
        <input
          ...
          style={{ flex: 3, fontSize: 13, padding: 10 }}
        />
        <input
          type="text"
          placeholder="학년"
...
function EnterRoomForm({ room, onEnter, onCancel, showToast }) {
  ...
  return (
    ...
    >
      ...
      />
      {error && (
        <div style={{ color: '#ff4444', fontSize: 12, marginBottom: 8 }}>
          [ {error} ]
        </div>
      )}
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
    ...
    <div className="overlay" onClick={onClose}>
      ...
      >
        ...
        >
          ...
          <h3 style={{ margin: 0, color: 'var(--point)', fontSize: 20 }}>
            ⚔️ 퀘스트 보드 입장
          </h3>
        </div>
          ...
          {loading ? (
            ...
            rooms.map((room) => (
              <div
                key={room.id}
                style={{
                  background: enteringRoom?.id === room.id ? '#0b0c0d' : 'var(--panel)',
              ...
              >
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, color: 'var(--text)' }}>
                    {room.school}{' '}
                    <span style={{ color: 'var(--point)' }}>
                      {room.grade}학년 {room.class}반
                    </span>
                  </div>
                  <div style={{ fontSize: 11, opacity: 0.3, marginTop: 3 }}>
                    {room.created_at}
                  </div>
                </div>
                ...
                <button
                  onClick={() =>
                    setEnteringRoom(enteringRoom?.id === room.id ? null : room)
                  }
                  style={{
